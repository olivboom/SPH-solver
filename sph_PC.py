...
class SPH_main(object):
    ...
    def calc_aD_PC(self, part, other_part, r_ij, mag_r_ij):
        mj = other_part.m
        q = mag_r_ij / self.h
        dwdr = dw_dr(q, self.h)
        pre_fac = mj * dwdr / mag_r_ij

        inv_rhoi2 = part.rho_ ** -2
        inv_rhoj2 = other_part.rho_ ** -2
        v_ij = part.v_ - other_part.v_
        fac2_1 = self.mu * (inv_rhoi2 + inv_rhoj2) * v_ij
        Pi = part.P_
        Pj = other_part.P_
        fac2_2 = ((Pi * inv_rhoi2) + (Pj * inv_rhoj2)) * r_ij
    ...
    def neighbour_iterate(self, part, PC):
                ...
                for other_part in self.search_grid[i, j]:
                    if part is not other_part:
                        if not PC:
                            ...
                            r_ij = part.x_ - other_part.x_
                        mag_r_ij = np.sqrt((r_ij[0] ** 2) + (r_ij[1] ** 2))
                        if mag_r_ij < 2 * self.h:# and other_part.boundary == False:
                            if not PC:
                                a, D = self.calc_aD(part, other_part, r_ij, mag_r_ij)
                            elif PC:
                                a, D = self.calc_aD_PC(part, other_part, r_ij, mag_r_ij)
                            part.a = part.a + a
                            part.D = part.D + D
    ...
    def forward_wrapper(self):
        ...
        while t < self.t_max:
            i = i + 1
            j = j + 1
            with open('State2.npy', 'wb') as fp:
                pickle.dump(self.particle_list, fp)
            with open('State2.npy', 'rb') as fp:
...
#                i = 0
            ...
            # plot the domain
            for particle in self.particle_list:
                self.neighbour_iterate(particle, PC=False)

            for particle in self.particle_list:
                particle.update_values_PC(self.B, self.rho0, self.gamma, self.dt/2)

            for particle in self.particle_list:
                self.neighbour_iterate(particle, PC=True)
...
class SPH_particle(object):
    ...
    def update_values(self, B, rho0, gamma, dt):
        ...
        if self.boundary == False:
            self.x = self.x_ + (self.v_ * dt)
            self.v = self.v_ + (self.a_ * dt)

            # if (self.x[1]<self.main_data.h):
            #     print(self.a,' ',self.P,' ',self.rho)

        self.rho = self.rho_ + (self.D_ * dt)
        prefactor = self.rho_ / rho0
        self.P = (prefactor ** gamma - 1) * B

    def update_values_PC(self, B, rho0, gamma, dt):
        """Updates the state of the particle for one time step forwards"""
        if self.boundary == False:
            self.x_ = self.x + (self.v * dt)
            self.v_ = self.v + (self.a * dt)

            # if (self.x[1]<self.main_data.h):
            #     print(self.a,' ',self.P,' ',self.rho)

        self.rho_ = self.rho + (self.D * dt)
        prefactor = self.rho_ / rho0
        self.P_ = (prefactor ** gamma - 1) * B
